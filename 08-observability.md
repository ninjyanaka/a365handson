...
> [!IMPORTANT]
> **3つの利用者向け画面（管理センター／Defender の agent-activity 画面／Purview）は、すべて Run の root にある `invoke_agent` span に依存**します。`invoke_agent` を出さないエージェントは、子 span（`chat`/`execute_tool`）を送っていても Defender Advanced Hunting の `CloudAppEvents` テーブルには残りますが、それ以外の画面には一切現れません。
...
4. **ツール呼び出し確認** — [Microsoft Defender portal](https://security.microsoft.com/) › **Investigation & response › Hunting › Advanced hunting** で `CloudAppEvents` テーブルを次の KQL で照会する（ツール実行・推論・MCP サーバー実行を種別ごとに確認）。
   ...
   直近で実行した Run の `traceId`（＝`ConversationId`）がヒットし、`ActionType` ごとに `invoke_agent`／`chat`／`execute_tool` の実行が確認できれば送信成功です（詳しいクエリは後述の「Defender Advanced Hunting」セクションを参照）。
