# Microsoft Agent 365 ハンズオン

自前ホスト型エージェントを **理解 → 準備 → 登録 → 認証 → 公開 → 統制 → 観測** まで実機で動かすハンズオンです。
...
| 01 | [Step 1：前提](./01-prerequisites.md) | 3 レイヤーの理解 / Blueprint→Instance / 事前準備・ポート設計 |
| 02 | [Step 2：Entra Agent ID](./02-entra-agent-id.md) | Entra Agent ID とは / 4 オブジェクトの関係 / Agent Registry |
| 03 | [Step 3：サードパーティ管理](./03-third-party-management.md) | **a365 CLI で自前 / 3P を管理下に**・コマンド一式・`setup all` の内部・Registry Sync |
