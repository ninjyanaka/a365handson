...
[Step 8](./08-observability.md) で送信した Agent 365 の観測データ（span／テレメトリ）が、**Microsoft Defender**（脅威検知・ランタイム保護）・**Microsoft Purview**（DLP・Insider Risk Management・監査などのデータガバナンス）・**Microsoft Entra Global Secure Access**（エージェント通信のネットワーク境界制御）にどう使われるかを確認します。観測が「何が起きたかを記録する」層だとすると、この Step は「危険な操作を止める・データ漏えいを防ぐ・不正な宛先への通信を遮断する・コンプライアンスを証明する」層です。
