# Step 8 — 管理・ガバナンス（ポリシー / CA / ライフサイクル）

[← 目次](./README.md) ｜ [← Step 7：観測](./07-observability.md) ｜ [次：Step 9 セキュリティ →](./09-security.md)

> [!TIP]
> **CA・Kill Switch・ライフサイクルを実機で検証したい場合（KQL / Graph / CLI 含む）は [Step 8 実習ラボ](./08-governance-lab.md) を使ってください。** レジストリ棚卸し・条件付きアクセス・キルスイッチ・`a365 cleanup` を、サインインログとクエリで「効くこと」まで検証するエンジニア向け教材です。本編（このページ）は概念・ポリシーテンプレート・ライフサイクルの考え方を扱います。
...
[← Step 7：観測](./07-observability.md) ｜ [次：Step 9 セキュリティ →](./09-security.md)
